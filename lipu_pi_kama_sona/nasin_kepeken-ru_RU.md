...
```ilo li sina
    ...
    tokiELinja(a" строка") # Правильно.
    b = " строка"
    tokiELinja(a b) # Правильно.
    tokiELinja(ab) # Неправильно. Переменная не "a" и "b", а "ab", которую не инициализировало.
```
...
Приостанавливает программу на время в миллисекундах, полученное из суммы данных времен. Строки времен должны быть похожи на целые числа (как "400" или "1369").
...
Премещает на метку, если все строки пустые.

#### **pokiPiLawaOS(\[имена...\]) -> строка в переменной среды или ничего**
...
Имя пользователя к примеру сохраняется в "USER" или "LOGNAME" на Линуксе, а в "USERNAME" на Windows. А что, если хотите, чтобы работало на Линуксе и Windows? То просто напишпите все варианты как аргументы функцию pokiPiLawaOS() (только для примера, использовуйте предварительно инициализированную переменную __nimi_jan вместо этого).

Если под именами отсуствует переменной или строка в них пустая, то возвращает пустую строку.

### ***Переходы***

ilo li sina использует GOTO (tawa(), niLaTawa() и alaLaTawa()), чтобы перемещать по программе.
...
Prodolzhit:
    ...
    niLaTawa(Prodolzhit "продолжить" "остановить" \
             "Продолжить?")
...
PoluchitImya:
    imya = kamaJoTanJan("Как вас зовут?")
    alaLaTawa(ImyeniNyet imya)
    tokiELinja("Здравствуйте, " imya "!")
```
...
```ilo li sina
    ...
    vyeshch = "вещь"
Bolshye:
    niLaTawa(Khvatit "нет" "да"       \
             "Вы хотите больше вещей?")
    vyeshch = wan(vyeshch vyeshch)
    tawa(Bolshye)
Khvatit:
    tokiELinja(vyeshch) # Если пользователь выбирал "да" 4 раза, то скажет "вещьвещьвещьвещьвещьвещьвещьвещьвещьвещьвещьвещьвещьвещьвещьвещь".
```
...
Вот почему: все языка - IO. Если нет ввод, то ничего не может сделать. Хотя, это и позволяет чтение из файлов. К примеру, если вы бы создали файл с именем "temp.txt", написали текста в нем и запустил "temp.txt | ilo_li_sina \<имя прогаммы\>", то программа получил бы ввод из него без бесконечного цикла.
