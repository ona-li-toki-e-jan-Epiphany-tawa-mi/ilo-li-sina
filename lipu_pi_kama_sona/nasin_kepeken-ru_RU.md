...
Вызывает niLaTawa() с сообщениям. Блокирует до получения ввода пользователя. Если пользователь отвечал сообщения да, то перемещает на метку, а если отвечал сообщение нет, то нет. Возвращает ввод от пользователя.
...
СпрошиватьСнова: 
    ...
    alaLaTawa(СпрошиватьСнова niLaTawa(Конец "Я люблю яблока" _                        \
                                    любитЯблока "? Не любишь яблока? Да ладно! Скажи " \
                                    "\"Я люблю яблока\" прям щас!"))
